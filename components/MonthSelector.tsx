...
function MonthSelector({
  ...
  return (
    <nav
      className="rounded-lg shadow p-0.5 bg-linear-to-r from-border to-primary/10"
      aria-label="Navegación del calendario"
    >
      <div className='flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 bg-card p-2 rounded-lg'>
        {/* Título - Arriba en mobile, centro en desktop */}
        <div className="w-full sm:w-auto text-center sm:order-0">
          <h2
            className="text-lg sm:text-xl font-bold mx-4 text-foreground"
            aria-live="polite"
            aria-atomic="true"
          >
            ...
            {isCurrent && (
              <span
                className="ml-2 text-xs font-normal text-primary-foreground px-1.5 py-0.5 rounded-md bg-primary"
